...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\mustang
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\mustang
    // Mac: ~/Library/Application Support/mustang
    // Unix: ~/.mustang
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "mustang";
#else
    fs::path pathRet;
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
...

   {
		...
		fprintf(ConfFile, "daemon=1\n");
				...
				fprintf(ConfFile, "addnode=185.133.192.242\n");
				fprintf(ConfFile, "addnode= 185.122.58.95\n");
				fprintf(ConfFile, "addnode=216.189.151.137\n");
                fclose(ConfFile);
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "mustangd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong mustang will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("mustang"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
